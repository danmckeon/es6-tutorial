...
var calculateAmortization = function calculateAmortization(principal, years, rate) {
    ...
    var balance = principal; //initialize balance to principal
    var amortization = []; //initialize amortization to empty arr
    for (var y = 0; y < years; y++) {
        var interestY = 0; //initialize interest pmt for year y
        var principalY = 0; //initialize principal pmt for year y
        for (var m = 0; m < 12; m++) {
            var interestM = balance * monthlyRate; //interest pmt for month m
            var principalM = monthlyPayment - interestM; //principal pmt for month m
            interestY = interestY + interestM;
            principalY = principalY + principalM;
            balance = balance - principalM;
        }

        amortization.push({ principalY: principalY, interestY: interestY, balance: balance }); //pass json to amortization array for each year
    }
